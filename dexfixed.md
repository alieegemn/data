		...
		currentRightClickMenu = CreateRightClickMenu(
			...
			function(option)
				if option == "Delete" then
					...
					Selection:Set({})
				elseif option == "Destroy Welds" then
					if not Option.Modifiable then return end
					local list = Selection:Get()
					for i,v in pairs(list) do
						for _,instance in pairs(v:GetDescendants()) do
							if not (instance:IsA("Weld") or instance:IsA("Attachment") or instance:IsA("Motor6D")) then continue end
							delete(instance)
						end
					end
				...
				elseif option == "Teleport To" then
					...
					for i = 1,#list do
						pcall(function()
							...
							if v:IsA("Model") then
								local rootPart = v.PrimaryPart or v:FindFirstChildOfClass("BasePart")
								if rootPart then hrp.CFrame = rootPart.CFrame end
							else
								hrp.CFrame = v.CFrame
							end
						end)
...
--]]
	...
	bindSelectionChanged:connect(function()
		showProperties(GetSelection())
	end)
		...
		while task.wait() do
			...
			if not sel then
				...
				for i,v in pairs(sel) do
					if oldSelection[i] ~= v then sameTable = false break end
				end
...
D_E_X.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui")
